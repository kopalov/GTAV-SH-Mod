...
function GUI.updateSelection()
	if(get_key_pressed(Keys.NumPad2)) or (CONTROLS.IS_CONTROL_PRESSED(2,67)) then
		if(GUI.selection < GUI.buttonCount -1  )then
			GUI.selection = GUI.selection +1
			GUI.time = 0
		end
	elseif (get_key_pressed(Keys.NumPad8)) or (CONTROLS.IS_CONTROL_PRESSED(2,3)) then
		if(GUI.selection > 0)then
			GUI.selection = GUI.selection -1
			GUI.time = 0
		end
	elseif (get_key_pressed(Keys.NumPad5)) or (CONTROLS.IS_CONTROL_PRESSED(2,70)) then
...
function GUI.renderButtons()
	...
	UI.SET_TEXT_FONT(1)
 	...
 	UI.SET_TEXT_EDGE(0, 0, 0, 0, 0)
 	UI._SET_TEXT_ENTRY("STRING")
 	UI._ADD_TEXT_COMPONENT_STRING("The Safehouse Mod")
 	UI._DRAW_TEXT(0.01+ 0.01, (0.025  + 0.00 - 0.0125 ))
 	GUI.renderBox(0.01,0.31,0.0,0.13 - 0.00,52,93,149,240)
 	--GUI.renderBox(0.01,0.31,0.025,0.035 - 0.00,35,48,19,250)
end
